## 👋 Olá, eu sou o Mário Novaes!

**Desenvolvedor Web & Cientista de Dados | Python • Django • PHP • Laravel • Docker • Power BI |  
Sistemas de Gestão, Intranets e Dashboards para Tomada de Decisão**

Sou desenvolvedor web e cientista de dados com atuação em **sistemas de gestão**, **intranets**, **dashboards analíticos** e **projetos educacionais**, aplicando tecnologia, dados e estatística para apoiar a **tomada de decisão em contextos reais**, incluindo educação, pesquisa e agronegócio.

- 💻 Desenvolvimento de sistemas web e intranets  
- 📊 Dashboards e análise de dados com Power BI  
- 🧠 Estatística aplicada a indicadores e métricas  
- 🌱 Tecnologia aplicada à educação, pesquisa e agro  

---

## 🛠️ Linguagens & Ferramentas

<p align="left">
  <img src="https://skillicons.dev/icons?i=python,django,php,laravel,vue,html,css,js,tailwind,docker,postgres,mysql,git,github&theme=dark" />
</p>

<p align="left">
  <img src="https://img.shields.io/badge/Inertia.js-9553E9?style=for-the-badge&logo=inertia&logoColor=white"/>
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
</p>

---

## 📊 Atividade no GitHub

<img src="https://github-readme-activity-graph.vercel.app/graph?username=MarioTecUfob&theme=tokyo-night&hide_title=true" />


## 📌 Projetos em Destaque

🔒 **Sistema de Gestão para Laboratório de Fitopatologia** *(Projeto Privado)*  
Sistema web desenvolvido para apoiar a gestão de rotinas laboratoriais em fitopatologia, incluindo organização de dados, acompanhamento de atividades e suporte à tomada de decisão baseada em dados.  
**Tecnologias:** PHP, MySQL, Docker.

🔒 **Sistema para Inspeção da Qualidade da Banana** *(Projeto Privado)*  
Sistema desenvolvido para registro, análise e acompanhamento de critérios de qualidade da banana, auxiliando processos de inspeção, padronização e tomada de decisão no contexto do agronegócio.  
**Tecnologias:** Laravel, MariaDB, Docker.

🔒 **Laranjeira Intranet** *(Projeto Privado)*  
Sistema de gestão interna desenvolvido em Django e Docker para organização de documentos, módulos administrativos e acompanhamento de atividades.

🌐 **Sites Institucionais para Associações**  
Desenvolvimento de sites institucionais para associações e organizações comunitárias, com foco em **divulgação**, **organização da informação**, **transparência** e **presença digital**.  
Inclui páginas institucionais, áreas de notícias, galerias, formulários de contato e integração com redes sociais.  
**Tecnologias:** PHP, Laravel, Vue.js, Tailwind CSS.

📊 **Dashboards & BI**  
Desenvolvimento de dashboards em **Power BI** aplicados à gestão administrativa e educacional, com foco em indicadores, métricas e apoio à tomada de decisão.

🎓 **Projetos Educacionais**  
Ferramentas e sistemas aplicados ao ensino de Matemática e Tecnologia, integrando programação, análise de dados e educação.

---

## 📫 Onde me encontrar

<p align="left">
  <a href="https://www.linkedin.com/in/mariowebtec" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:SEUEMAIL@gmail.com">
    <img src="https://img.shields.io/badge/E--mail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

⭐ Sempre aberto a colaborar, trocar conhecimento e desenvolver soluções que gerem impacto real.
